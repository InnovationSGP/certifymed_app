...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        // Check if currently locked out before proceeding
        const lockoutUntil = localStorage.getItem('lockoutUntil');
        if (lockoutUntil && new Date(lockoutUntil) > new Date()) {
            const timeRemaining = Math.ceil(
                (new Date(lockoutUntil) - new Date()) / 60000
            );
            toast.error(
                `Too many failed attempts. Please try again in ${timeRemaining} minutes.`,
                {
                    duration: 5000,
                    position: 'top-center'
                }
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (setAuth(userData)) {
                    ...
                    // Clear failed attempts and lockout time
            ...
            const currentFailedAttempts = parseInt(
            ...
            // Always disable button for 3 seconds after any failed attempt
            setIsButtonDisabled(true);
            const disabledUntil = new Date(Date.now() + 3000);
            localStorage.setItem(
                'buttonDisabledUntil',
            ...
            setTimeout(() => {
                // Only remove the disabled state if we're not in a lockout period
            ...
            if (newFailedAttempts >= 3) {
                ...
                const lockoutUntil = new Date(Date.now() + 300000); // 5 minutes
        ...
        <section className="h-full w-full max-w-[542px]">
            <div className="min-w-full mx-auto flex flex-col h-full">
                <div className="grow h-full">
                    ...
                    <form onSubmit={handleLogin}>
                        ...
                        <PrimaryBtn
                            ...
                            disabled={isLoggingIn || isButtonDisabled}
                        >
                ...
                <p className="text-center font-medium py-5">
                    ...
                    <TransitionLink
                        href="/sign-up"
                        className="text-spandexGreen pl-1"
                    >
                        Sign up
                    </TransitionLink>
                </p>
