...
const SignUp = ({ role }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const userType =
                role === 'doctor' ? 'CARE_COORDINATOR' : 'CUSTOMER';
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                const userData = {
                    ...
                    firstName:
                        responseData.user?.firstName || responseData.firstName,
                    lastName:
                        responseData.user?.lastName || responseData.lastName,
                    email: responseData.role?.email || responseData.email,
                    phoneNumber:
                        responseData.user?.phoneNumber ||
                        responseData.phoneNumber,
                    countryCode:
                        responseData.user?.countryCode || formData.countryCode,
                    countryName:
                        responseData.user?.countryName || formData.countryName,
                    gender: responseData.user?.gender || formData.gender,
                    dateOfBirth:
                        responseData.user?.dateOfBirth || formData.dateOfBirth,
                    createdAt:
                        responseData.user?.createdAt || responseData.createdAt,
                    updatedAt:
                        responseData.user?.updatedAt || responseData.updatedAt
                };
