            ...
            {sidebarLinks &&
                sidebarLinks.map((obj, index) => (
                    <button
                        ...
                        } flex items-center justify-center xl:justify-start space-x-1 cursor-pointer sm:space-x-2 text-white font-medium text-[11px] sm:text-sm font-poppins w-full px-2 sm:px-3.5 py-2 sm:py-[13px] rounded-xl transition-all duration-300 ease-in-out`}
                    >
                        {obj.icon}

                        {obj.url === pathname && (
                            <span className="xl:hidden whitespace-nowrap transition-all duration-300 ease-in-out cursor-pointer">
                                {obj.name}
                            </span>
                        )}
                        <span className="hidden xl:block transition-all duration-300 ease-in-out cursor-pointer">
                            {obj.name}
                        </span>
                    </button>
                ))}
